...
at::Tensor grouped_mlp_activation_forward(
    ...
    MegacppActivation activation, int64_t glu_interleave_size, double activation_limit,
  ...
  NVTE_CHECK(scales.numel() == input.size(0),
             "act_scales must have one value per activation row.");
  ...
  NVTE_SCOPED_GIL_RELEASE({
    switch (activation) {
      case MegacppActivation::kSwiGLU:
        nvte_scaled_swiglu(te_input.data(), te_scales.data(), te_output.data(),
...
std::vector<at::Tensor> megacpp_grouped_mlp_forward(
  ...
  auto fc2_x = grouped_mlp_activation_forward(
      fc1_preact, act_scales, activation_kind, glu_interleave_size, activation_limit,
