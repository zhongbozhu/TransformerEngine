...
def bulk_alloc_float8_blockwise_tensor(inp_view, m_splits, quantizer_list):
    ...
    for i in range(len(quantizer_list)):
        out_tensor_list.append(Float8BlockwiseQTensorBase(
            rowwise_data=out_rowwise_data_list[i],
            rowwise_scale_inv=out_rowwise_scale_list[i],
            columnwise_data=out_columnwise_data_list[i],
            columnwise_scale_inv=out_columnwise_scale_list[i],
            fp8_dtype=fp8_dtype,
