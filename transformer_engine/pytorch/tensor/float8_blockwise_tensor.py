...
# # TODO: move this function into pytorch C++ layer
# def bulk_alloc_float8_blockwise_tensor(inp_view, m_splits, quantizer_list):
#     """
#     Bulk allocate the float8 blockwise tensor for the given quantizer list.

#     Should improve CPU overhead issue for grouped linear 
#     """
#     # assumes all the quantizers have the same usage
#     rowwise_data_shapes = []
#     rowwise_scale_shapes = []
#     columnwise_data_shapes = []
#     columnwise_scale_shapes = []
#     is_2D_scaled = quantizer_list[0].block_scaling_dim == 2
#     fp8_dtype = quantizer_list[0].dtype
#     use_rowwise = quantizer_list[0].rowwise_usage
#     assert use_rowwise, "bulk alloc needs to have rowwise usage"
#     use_columnwise = quantizer_list[0].columnwise_usage
#     out_elem_size = 1
#     out_scale_size = 4
#     M, K = inp_view.shape
#     for i, quantizer in enumerate(quantizer_list):
#         inp_view_i_shape = (m_splits[i], K)
#         rowwise_data_shapes.append(inp_view_i_shape)
#         rowwise_scale_shapes.append(quantizer.get_scale_shape(inp_view_i_shape, False))
#         if use_columnwise:
#             columnwise_data_shapes.append((K, m_splits[i]))
#             columnwise_scale_shapes.append(quantizer.get_scale_shape(inp_view_i_shape, True))

#     # print(f"[DEBUG] rowwise_data_shapes: {rowwise_data_shapes}")
#     # print(f"[DEBUG] rowwise_scale_shapes: {rowwise_scale_shapes}")
#     # print(f"[DEBUG] columnwise_data_shapes: {columnwise_data_shapes}")
#     # print(f"[DEBUG] columnwise_scale_shapes: {columnwise_scale_shapes}")

#     # now for all the rowwise data, allocate one giant tensor
#     # TODO: need alignment, each tensor should be aligned at 128 or 16 byte address, this can be done by padding

#     rowwise_data_sizes = []
#     rowwise_scale_sizes = []
#     for i in range(len(rowwise_data_shapes)):
#         rowwise_data_sizes.append(rowwise_data_shapes[i][0] * rowwise_data_shapes[i][1] * out_elem_size)
#         rowwise_scale_sizes.append(rowwise_scale_shapes[i][0] * rowwise_scale_shapes[i][1] * out_scale_size)
#     total_size_rowwise_data = sum(rowwise_data_sizes)
#     total_size_rowwise_scale = sum(rowwise_scale_sizes)
#     # now use torch.empty to allocate one giant tensor for rowwise data and scale
#     total_size = total_size_rowwise_data + total_size_rowwise_scale
#     rowwise_full_tensor = torch.empty(total_size, dtype=torch.uint8, device=inp_view.device)
#     # now slice the rowwise data and scale one by one
#     rowwise_data_chunk = rowwise_full_tensor[:total_size_rowwise_data]
#     rowwise_scale_chunk = rowwise_full_tensor[total_size_rowwise_data:total_size_rowwise_data+total_size_rowwise_scale]
#     out_rowwise_data_list = []
#     out_rowwise_scale_list = []
#     start_data = 0
#     start_scale = 0
#     for i in range(len(rowwise_data_shapes)):
#         out_rowwise_data_list.append(rowwise_data_chunk[start_data:start_data+rowwise_data_sizes[i]].view(rowwise_data_shapes[i]))
#         out_rowwise_scale_list.append(rowwise_scale_chunk[start_scale:start_scale+rowwise_scale_sizes[i]].view(dtype=torch.float32).view(rowwise_scale_shapes[i]))
#         start_data += rowwise_data_sizes[i]
#         start_scale += rowwise_scale_sizes[i]

    
#     if use_columnwise:
#         total_size_columnwise_data = 0
#         total_size_columnwise_scale = 0
#         columnwise_data_sizes = []
#         columnwise_scale_sizes = []
#         for i in range(len(columnwise_data_shapes)):
#             columnwise_data_sizes.append(columnwise_data_shapes[i][0] * columnwise_data_shapes[i][1] * out_elem_size)
#             columnwise_scale_sizes.append(columnwise_scale_shapes[i][0] * columnwise_scale_shapes[i][1] * out_scale_size)
#         total_size_columnwise_data = sum(columnwise_data_sizes)
#         total_size_columnwise_scale = sum(columnwise_scale_sizes)
#         total_size = total_size_columnwise_data + total_size_columnwise_scale
#         columnwise_full_tensor = torch.empty(total_size, dtype=torch.uint8, device=inp_view.device)
#         # now slice the columnwise data and scale one by one
#         columnwise_data_chunk = columnwise_full_tensor[:total_size_columnwise_data]
#         columnwise_scale_chunk = columnwise_full_tensor[total_size_columnwise_data:total_size_columnwise_data+total_size_columnwise_scale]
#         out_columnwise_data_list = []
#         out_columnwise_scale_list = []
#         start_data = 0
#         start_scale = 0
#         for i in range(len(columnwise_data_shapes)):
#             out_columnwise_data_list.append(columnwise_data_chunk[start_data:start_data+columnwise_data_sizes[i]].view(columnwise_data_shapes[i]))
#             out_columnwise_scale_list.append(columnwise_scale_chunk[start_scale:start_scale+columnwise_scale_sizes[i]].view(dtype=torch.float32).view(columnwise_scale_shapes[i]))
#             start_data += columnwise_data_sizes[i]
#             start_scale += columnwise_scale_sizes[i]

#     # now assembly these data into Float8BlockwiseQTensorBase 
#     out_tensor_list = []
#     for i in range(len(quantizer_list)):
#         out_tensor_list.append(Float8BlockwiseQTensorBase(
#             rowwise_data=out_rowwise_data_list[i] if use_rowwise else None,
#             rowwise_scale_inv=out_rowwise_scale_list[i] if use_rowwise else None,
#             columnwise_data=out_columnwise_data_list[i] if use_columnwise else None,
#             columnwise_scale_inv=out_columnwise_scale_list[i] if use_columnwise else None,
#             fp8_dtype=fp8_dtype,
