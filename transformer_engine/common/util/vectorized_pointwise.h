...
template <int nvec, bool aligned, typename ComputeType, typename Param,
          ComputeType (*OP)(ComputeType, const Param &), bool IS_CURRENT_SCALING, typename InputType, typename OutputType>
...
template <int nvec, bool aligned, typename ComputeType, typename Param,
          ComputeType (*OP)(ComputeType, const Param &), bool IS_CURRENT_SCALING, typename InputType, typename InputTypeGrad,
          typename OutputType>
__launch_bounds__(unary_kernel_threads) __global__
...
template <int nvec, typename Param, fp32 (*OP)(const fp32, const Param &), bool IS_CURRENT_SCALING, typename InputType,
          typename OutputType>
  ...
  if (N != 0) {
    ...
    switch (align) {
      case Alignment::SAME_ALIGNED:
        unary_kernel<nvec, true, fp32, Param, OP, IS_CURRENT_SCALING><<<num_blocks, threads, 0, stream>>>(
            input, noop, output, scale, amax, scale_inv, params, N, num_aligned_elements);
        break;
      case Alignment::SAME_UNALIGNED:
        unary_kernel<nvec, false, fp32, Param, OP, IS_CURRENT_SCALING><<<num_blocks, threads, 0, stream>>>(
            input, noop, output, scale, amax, scale_inv, params, N, num_aligned_elements);
        break;
      case Alignment::DIFFERENT: {
        // If the pointers are aligned differently we cannot vectorize
        unary_kernel<1, true, fp32, Param, OP, IS_CURRENT_SCALING><<<num_blocks, threads, 0, stream>>>(
...
template <int nvec, typename Param, fp32 (*OP)(fp32, const Param &), bool IS_CURRENT_SCALING, typename InputType,
...
void VectorizedUnaryGradKernelLauncher(const InputTypeGrad *grad, const InputType *input,
  ...
  if (N != 0) {
    ...
    switch (align) {
      case Alignment::SAME_ALIGNED:
        unary_grad_kernel<nvec, true, fp32, Param, OP, IS_CURRENT_SCALING><<<num_blocks, threads, 0, stream>>>(
            grad, input, output, scale, amax, scale_inv, params, N, num_aligned_elements);
        break;
      case Alignment::SAME_UNALIGNED:
        unary_grad_kernel<nvec, false, fp32, Param, OP, IS_CURRENT_SCALING><<<num_blocks, threads, 0, stream>>>(
            grad, input, output, scale, amax, scale_inv, params, N, num_aligned_elements);
        break;
      case Alignment::DIFFERENT: {
        // If the pointers are aligned differently we cannot vectorize
        unary_grad_kernel<1, true, fp32, Param, OP, IS_CURRENT_SCALING><<<num_blocks, threads, 0, stream>>>(
...
__device__ __forceinline__ void compute_scale_from_amax(fp32 amax, fp32* scale_ptr, fp32* scale_inv_ptr, 
...
void ComputeScaleFromAmaxKernelLauncher(fp32* amax_ptr, fp32* scale_ptr, fp32* scale_inv_ptr, 
                                        const fp32 max_fp8, const fp32 epsilon,
                                        cudaStream_t stream) {
  compute_scale_from_amax_kernel<USE_IEEE_DIV> <<<1, 1, 0, stream>>>(amax_ptr, scale_ptr, scale_inv_ptr, max_fp8, epsilon);
}

template <int nvec, bool aligned, typename InputType>
__launch_bounds__(unary_kernel_threads) __global__
    void unary_kernel_amax(const InputType *input, fp32 *amax,
                          const size_t N, const size_t num_aligned_elements) {
  VectorizedLoader<InputType, nvec, aligned> loader(input, N);
  InputType max = 0;
  const int warp_id = threadIdx.x / THREADS_PER_WARP;
