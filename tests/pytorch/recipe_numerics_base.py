...
class TestFP8RecipeLinearBase:
    ...
    def _check_golden_tensor_dumps(dump_dir, get_recipe, dims, input_dtype):
        ...
        scaling_type = "ScalingType.PER_TENSOR"  # Assuming the scaling type is PER_TENSOR for this example
    ...
    def run_linear_preprocess_parallel(
        ...
        if tp_size > 1:
            if parallel_mode == "column":
                ...
                bias = (
                    cls._shard_tensor(bias, tp_size, 0)[rank]
                    if bias is not None
                    else None
                )
    ...
    def run_linear_postprocess_parallel(
        ...
        if tp_size > 1:
            if parallel_mode == "column":
                ...
                bgrad = (
                    cls._gather_tensor(bgrad, tp_size, tp_group, 0)
            ...
            if parallel_mode == "row":
                ...
                if sequence_parallel:
                    ...
                    bgrad = cls._all_reduce_tensor(bgrad, tp_size, tp_group) if bgrad is not None else None
    ...
    def compare_recipe(
        ...
        if recipe2_golden_tensors is not None:
            print(y_q)
            torch.testing.assert_close(y_q.float(), recipe2_golden_tensors["y"].float(), atol=0.25, rtol=0.0)
            ...
            if use_bias:
                torch.testing.assert_close(bgrad, recipe2_golden_tensors["bgrad"], atol=0.0, rtol=0.0)
