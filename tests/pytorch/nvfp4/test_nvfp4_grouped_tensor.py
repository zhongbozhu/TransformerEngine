...
def fused_grouped_quantize(x: torch.Tensor, split_section_tensor: torch.Tensor, quantizer: NVFP4Quantizer):
    # ideally we want to call tex.contiguous_grouped_quantize(x, split_section_tensor, quantizer)
    ...
    # build shape as a list[(int, int)]
    shape = [(split_section_tensor[i].item(), hidden_dim) for i in range(num_tensors)]

    # build quantizers as a list[NVFP4Quantizer]
    quantizers = [quantizer] * num_tensors
    ...
    # copy input data to grouped input
    grouped_input.data.copy_(x.reshape(-1))
    ...
    _ = tex.quantize_grouped(grouped_input, grouped_output, quantizer)
