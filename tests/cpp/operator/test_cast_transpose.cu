...
template <typename InputType, typename OutputType>
void performTestCurrentScaling(const size_t N, const size_t H) {
  using namespace test;
  ...
  Tensor input("input", { N, H }, itype);
  Tensor output("output", { N, H }, otype, true, true, NVTE_CURRENT_TENSOR_SCALING);
  ...
  if (isFp8Type(otype)) {
    ...
    compareResults("amax", output.amax(), ref_amax, 0.0f, rtol_fp32);
...
TEST_P(CTTestSuite, TestCastTranspose) {
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_ALL(input_type, InputType,
    TRANSFORMER_ENGINE_TYPE_SWITCH_ALL(output_type, OutputType,
      ...
      performTestCurrentScaling<InputType, OutputType>(size.first, size.second);
    );
  );
}
